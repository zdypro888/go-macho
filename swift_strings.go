...
func legacySwiftObjCClassName(fullName string) (string, bool) {
	...
	for len(encoded) > 0 {
		partLen, digits, ok := consumeDecimalPrefix(encoded)
		if !ok || digits == 0 || partLen <= 0 {
			return "", false
		}
		// Subtraction avoids overflowing digits+partLen for malformed names.
		if partLen > len(encoded)-digits {
			return "", false
		}
...
func consumeDecimalPrefix(input string) (value int, digits int, ok bool) {
	maxInt := int(^uint(0) >> 1)
